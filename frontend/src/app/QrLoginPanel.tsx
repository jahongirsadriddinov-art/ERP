...
export default function QrLoginPanel({ onLogin, onBack }: { onLogin: (u: any, company?: any) => void; onBack: () => void }) {
  ...
  const createSession = async () => {
    setScannedCount(0); setVerified(false); setError("");
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        const d = await r.json();
        if (stop) return;
        // "Tugadi" degan xatoni HECH QACHON ko'rsatmaymiz — sahifa ochiq
        // turgan ekan, QR "umuman tugamaydi": ichki sessiya muddati
        // yetganda shunchaki sezilmasdan YANGISINI so'raymiz.
        if (d.expired) { createSession(); return; }
  ...
  return (
    <div className="space-y-4 text-center">
      ...
      <div className="w-12 h-12 rounded-2xl mx-auto flex items-center justify-center shadow-lg shadow-primary/25"
        style={{ background: "linear-gradient(135deg, var(--primary) 0%, var(--accent) 100%)" }}>
        <MorphIcon icon={QrCode} className="w-6 h-6 text-white" />
      </div>
      ...
      <div className="relative w-[220px] h-[220px] mx-auto rounded-2xl overflow-hidden bg-white flex items-center justify-center ring-4 ring-primary/10">
        {error ? (
